...
const CustomTooltip = ({ active, payload, selectedTimeRange, selectedDifficulty }: CustomTooltipProps) => {
 ...
 */
export function TopicWisePerformanceChart({ history, selectedTopic }: TopicWisePerformanceChartProps) {
    ...
    const filteredHistory = topicHistory.filter(quiz => {
        ...
        const isYesterday = quizDate.toDateString() === new Date(now.setDate(now.getDate() - 1)).toDateString()
        ...
        const difficultyFilter = selectedDifficulty.value === "all" ||
            quiz.config.difficulty === selectedDifficulty.value
    ...
    const difficultyData = filteredHistory.reduce((acc, quiz) => {
    ...
    const barData = Object.entries(difficultyData)
        ...
        .sort((a, b) => {
            ...
            return order[a.difficulty.toLowerCase() as keyof typeof order] - order[b.difficulty.toLowerCase() as keyof typeof order]
        })
